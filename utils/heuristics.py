...
def heuristic_search_standard(trues, preds_a, preds_b, preds_score_a, preds_score_b, param_range, reverse, results, key_1, key_2):
    acc = []
    acc_ps = []
    ...
    if not reverse:
        for threshold in tqdm(param_range, total=len(param_range)):            
            ...
            acc.append({
                'threshold' : threshold,
                'accuracy' : max_p_correct / len(trues) * 100,
                'model_b_usage': max_p_model_b_usage / len(trues) * 100
            })

            acc_ps.append({
                'threshold' : threshold,
                'accuracy' : max_p_correct_ps / len(trues) * 100,
                'model_b_usage': max_p_model_b_usage / len(trues) * 100,
            })
    else:
        for threshold in tqdm(param_range, total=len(param_range)):
...
def heuristic_search_standard_2(trues, preds_a, preds_b, preds_score_a, preds_score_b, param_range, reverse, results, key_1, key_2):
    acc = []
    acc_ps = []
    # threshold_params = np.linspace(0.0001, 1, 1000)
    if not reverse:
